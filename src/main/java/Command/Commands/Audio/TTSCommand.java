...
public class TTSCommand extends DiscordCommand {
    ...
    private String getGoogleTTS(String content) {
        try {
            return "http://" + Secret.LOCAL_IP + Secret.LOCAL_API_PATH + "google/" + EmbedHelper.urlEncode(content);
        }
        catch(Exception e) {
            e.printStackTrace();
        }
    ...
    private String getDectalkTTS(String content) {
        try {
            return "http://" + Secret.LOCAL_IP + Secret.LOCAL_API_PATH + "dectalk/" + EmbedHelper.urlEncode(content);
        }
        catch(Exception e) {
            return null;
        }
