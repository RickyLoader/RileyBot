...
public abstract class SelfieCommand extends DiscordCommand {
     ...
     */
    public SelfieCommand(String trigger, String desc, String backgroundPath, String thumbnailUrl, ProfileDimensions profileDimensions) {
        super(trigger, desc);
    ...
    public void execute(CommandContext context) {
        ...
        new Thread(() -> {
            try {
                ...
                byte[] image = buildImage(context.getMember().getUser());
                EmbedBuilder builder = getEmbedBuilder(context).setImage("attachment://" + IMAGE_NAME);

                channel.sendMessage(builder.build()).addFile(image, IMAGE_NAME).queue();
            }
            catch(Exception e) {
     ...
     */
    private byte[] buildImage(User user) throws Exception {
